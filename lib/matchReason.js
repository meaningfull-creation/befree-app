...
function buildReasonSystemPrompt() {
  return `あなたはスタートアップ支援の専門家です。企業の課題と実務経験者の強みを照らし合わせ、なぜこの組み合わせが良いマッチなのかを、経営者が読んで納得できる日本語で説明します。誇張や一般論を避け、具体的な軸名に触れてください。良い点だけでなく、ミスマッチになり得る懸念点があれば正直に一言添えてください(無理に懸念点を作る必要はありません。データから読み取れる場合のみ)。必ず日本語で、JSON以外の文字を一切含まない出力のみを返してください。`;
}
...
タスク:
上記から、なぜこの人材がこの企業に合うのかを40〜70字程度で説明してください(一致軸に具体的に触れること)。
続けて、あれば懸念点(ミスマッチになり得る要素)を20〜40字程度で一言添えてください。データから明確に読み取れない場合は無理に書かず、空文字のままにしてください。
...
export async function generateMatchReason(companySkillMapId, talentSkillMapId, matchScore) {
  try {
    ...
    // Match.reasonは単一フィールドのため、推薦理由と懸念点(あれば)を1つの文字列に結合する。
    return result.caveat ? `${result.reason}(懸念点: ${result.caveat})` : result.reason;
  } catch (e) {
