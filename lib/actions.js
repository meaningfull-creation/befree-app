...
export async function acceptMatchAction(formData) {
  ...
  const match = await prisma.match.findUnique({
    ...
    include: {
      ...
      engagement: { include: { project: true } },
    },
  });
  if (!match) return;
  if (match.engagement?.status === "active" || match.engagement?.status === "completed") return; // 既に成立済みなら何もしない

  const { engagement, project } = await prisma.$transaction(async (tx) => {
    // 契約は企業側から提案する運用が基本だが、運営が代わりに直接成立させたい場合(fallback)のため、
    // 既に "proposed" の提案があればそれを引き継いで成立させ、なければ新規作成する。
    ...
    // 契約成立と同時に、企業⇄人材で共有するプロジェクトを自動作成する(まだ無ければ)。
    // 対象課題(targetAxis)は、企業の最新スキルマップで最もスコアが低い(深刻な)軸を引き継ぐ。
    let project = match.engagement?.project || null;
    if (!project) {
